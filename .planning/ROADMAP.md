...
**Plans**: 5 plans, in 5 waves (each depends on the one before — `Makefile`,
...
- [ ] 05-01-PLAN.md — The skip-token gate: a `commit-msg` hook (`scripts/skip_tokens.py`)
...
- [ ] 05-02-PLAN.md — `make pr.land PR=N`: lands a green PR with the checked head, subject
...
- [ ] 05-03-PLAN.md — The wall: the repository's `default` ruleset retargeted to `main`
  ...
  L22 appended *(wave 3)*
- [ ] 05-04-PLAN.md — Python 3.12 only: L23 (supersedes L01's floor); `requires-python`,
  ruff py312 and the three findings it forces; `make venv`, the CI matrix and the
  required list narrowed in one commit *(wave 4)*
- [ ] 05-05-PLAN.md — The record made true: every current document says 3.12 and cites the
