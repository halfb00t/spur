...
| Solid model | `src/spur/model.py` | the CadQuery solid, the kernel lock, the caches, STL/STEP export — see `solid-model/` |
| HTTP API + UI serving | `src/spur/app.py` | endpoints, admission control, static hosting — see `http-api.md` |
...
The dependency direction is enforced, not hoped for: `pyproject.toml` declares
import-linter contracts and `make verify` fails on a violation. `calc.py` must never
import the CAD kernel; the CLI must never import the web layer.

## Source of truth

- **The part** — nothing is stored. Every answer is derived from `GearParams` on demand;
  caches are a speed optimisation and are safe to lose (L07).
